...
@interface LEResourceLoaderManager()<AVAssetResourceLoaderDelegate,LERequestLoaderDelegate>
...
#pragma mark - LERequestLoaderDelegate
- (void)requestLoader:(LEResourceLoader *)loader didCompleteWithError:(NSError *)error {
    if ([self.delegate respondsToSelector:@selector(resourceLoaderManager:didCompleteWithError:)]) {
        [self.delegate resourceLoaderManager:self didCompleteWithError:error];
    }
}
